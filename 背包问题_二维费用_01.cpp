...
class Solution{
public:
    MaxPrice_01(int N, int V, int W, vector<int> &V_cost, vector<int> &W_cost, vector<int> &Price)
    {
        ...
        for(int k=1; k<=N; k++){// 第k件物品
            for(int i=V; i>=V_cost[k]; i--){
                for(int j=W; j>=W_cost[k]; j--){
                    dp[i][j] = max(dp[i][j], dp[i-V_cost[k]][j-W_cost[k]] + Price[k]);
                }
            }
        }
        return dp[V][W];
...
int main()
{
    ...
    Solution s;
    cout << s.MaxPrice_01(N, V, W, V_cost, W_cost, Price);
    return 0;
}
