...
class Solution{
public:
    MaxPrice_Multi(int N, int V, int W, vector<int> &V_cost, vector<int> &W_cost, vector<int> &Price, vector<int> Count)
    {
        ...
        for(int k=1; k<=N; k++){// 第k件物品
            for(int i=V; i>=V_cost[k]; i--){
                for(int j=W; j>=W_cost[k]; j--){
                    ...
                    max_start = min(max_start, Count[k]);
...
int main()
{
    ...
    vector<int> Count = {0, 3, 1, 1, 1};
    vector<int> Price = {0, 3, 4, 5, 6};
    Solution s;
    cout << s.MaxPrice_Multi(N, V, W, V_cost, W_cost, Price, Count);
    return 0;
}
